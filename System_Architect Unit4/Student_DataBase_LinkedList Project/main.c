...
/*Recursive  Method*/
int Number_Of_Students_UsingRecursion(struct SStudent* head)
{
	...
	return 1 + Number_Of_Students_UsingRecursion(head->PNextStudent) ;
...
void Reverse_List()
{
	...
	{
		Next = gpFirstStudent->PNextStudent;
		gpFirstStudent->PNextStudent = Prev;
		Prev = gpFirstStudent;
		gpFirstStudent = Next;
...
int main()
{
	...
	while(1)
	{
		...
		case 6:
			//Number_Of_Students();
			DPRINTF("\n the number of students is %d",Number_Of_Students_UsingRecursion(gpFirstStudent));
			break;
		case 7:
			Reverse_List();
